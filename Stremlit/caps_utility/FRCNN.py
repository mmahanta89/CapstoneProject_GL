...
def getAnnotatedData(trainPath, testPath, annoTrainPath, annoTestPath, numTrain = 20, numTest = 2):
    
    #setting for resizing all to uniform size and reducing memory utilization
    img_height=100
    img_width=100
    
    st.write('Innside function')
    ...
    carType = ''
    SubCounter = 0
    with open("annotationAlt.txt", "w+") as f:
    ...
    carType = ''
    SubCounter = 0
    with open("test_annotationAlt.txt", "w+") as f:
        for idx, row in df_test.iterrows():
            
            if(SubCounter >= numTest):
                if(carType == row['fol_details']):
                    SubCounter+=1
                    continue
                else:
